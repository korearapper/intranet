...
async def debug_naver(keyword:str):
    p=px()
    gql_url="https://pcmap-api.place.naver.com/graphql"
    payload=[{"operationName":"getPlacesList","variables":{"input":{"query":keyword,"start":1,"display":5,"adult":False,"spq":False,"queryRank":"","x":"126.9783882","y":"37.5666103","deviceType":"pcmap","bounds":""},"isNmap":True,"isBounds":False},"query":"query getPlacesList($input: PlacesInput, $isNmap: Boolean!, $isBounds: Boolean!) { businesses: places(input: $input) { total items { id name tel category address reviewCount } } }"}]
    headers={"Content-Type":"application/json","User-Agent":UA,"Referer":"https://pcmap.place.naver.com/","Origin":"https://pcmap.place.naver.com"}
    results=[]
    try:
        async with httpx.AsyncClient(proxy=p,timeout=20) as c:
            r=await c.post(gql_url,json=payload,headers=headers)
            results.append({"test":"proxy","proxy":p.split("@")[1],"status":r.status_code,"body":r.text[:2000]})
    except Exception as e:
        results.append({"test":"proxy","error":str(e)})
    try:
        async with httpx.AsyncClient(timeout=20) as c:
            r=await c.post(gql_url,json=payload,headers=headers)
            results.append({"test":"direct","status":r.status_code,"body":r.text[:2000]})
    except Exception as e:
        results.append({"test":"direct","error":str(e)})
