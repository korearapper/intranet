...
async def debug_parse(keyword:str):
    """m.search.naver.com HTML에서 업체 데이터 파싱 테스트"""
    p=px()
    ...
    try:
        async with httpx.AsyncClient(proxy=p,timeout=20,follow_redirects=True) as c:
            ...
            import json as jn
            results=[]
            # __NEXT_DATA__ 또는 window.__jindo_callback 등
            for pat in [r'<script[^>]*id="__NEXT_DATA__"[^>]*>(.*?)</script>',r'"place":\s*(\{[^}]{100,})',r'"items":\s*(\[[^\]]{100,}\])',r'data-nclick[^>]*class="[^"]*_place[^"]*"[^>]*>(.*?)</']:
                ms=re.findall(pat,html[:500000],re.DOTALL)
                if ms:
                    results.append({"pattern":pat[:40],"count":len(ms),"sample":ms[0][:500] if ms else ""})
            # 방법2: place ID 패턴
            pids=re.findall(r'/place/(\d{8,})',html)
            # 방법3: tel 패턴
            tels=re.findall(r'(\d{2,4}-\d{3,4}-\d{4})',html)
            # 방법4: data-sid (place id)
            sids=re.findall(r'data-sid="(\d+)"',html)
            # 방법5: JSON-LD
            jlds=re.findall(r'<script type="application/ld\+json">(.*?)</script>',html,re.DOTALL)
            return {"status":r.status_code,"html_len":len(html),"place_ids":list(set(pids))[:20],"data_sids":list(set(sids))[:20],"tels":list(set(tels))[:20],"json_ld_count":len(jlds),"json_ld_sample":jlds[0][:500] if jlds else "","patterns":results,"html_snippet_50k":html[50000:52000]}
    except Exception as e:
        return {"error":str(e)}
