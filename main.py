...
async def naver_search(keyword: str, proxy: str):
    """네이버 플레이스 GraphQL API로 검색 — 캡차 우회"""
    ...
    payload = [{
        ...
        "query": "query getPlacesList($input: PlacesInput, $isNmap: Boolean!, $isBounds: Boolean!) { businesses: places(input: $input) { total items { id name tel category address x y reviewCount blogCatalogReviewCount bookingReviewCount totalReviewCount virtualTel } } }"
    }]
    headers = {
        ...
        async with httpx.AsyncClient(proxy=proxy, timeout=20) as c:
            ...
            if r.status_code == 200:
                ...
                if isinstance(data, list) and data:
                    biz = data[0].get("data", {}).get("businesses", {})
                    if biz and biz.get("items"):
                        items = biz["items"]
                        return {"list": [{"id": it.get("id",""), "name": it.get("name",""), "tel": it.get("tel","") or it.get("virtualTel",""), "address": it.get("address",""), "category": [it.get("category","")] if it.get("category") else [], "reviewCount": it.get("reviewCount",0), "blogReviewCount": it.get("blogCatalogReviewCount",0)} for it in items], "totalCount": biz.get("total",0)}
    except:
        pass
    # 2차: 프록시 없이
    try:
...
# DEBUG - 네이버 검색 직접 테스트
gql_url_const="https://pcmap-api.place.naver.com/graphql"

@app.get("/api/debug/parse")
async def debug_parse(keyword:str):
    p=px()
    mobile_ua="Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15"
    try:
        async with httpx.AsyncClient(proxy=p,timeout=20,follow_redirects=True) as c:
            r=await c.get(f"https://m.search.naver.com/search.naver?query={urllib.parse.quote(keyword)}&sm=mtb_plc&where=m_local",headers={"User-Agent":mobile_ua})
            html=r.text
            import json as jn
            # 방법1: __typename Place 또는 Restaurant 블록
            places_json=re.findall(r'"__typename"\s*:\s*"(?:Place|Restaurant|LocalBusiness)"[^}]{50,500}',html)
            # 방법2: "id":"숫자","name":"업체명" 패턴
            id_name=re.findall(r'"id"\s*:\s*"(\d{5,})"\s*,\s*"name"\s*:\s*"([^"]+)"',html)
            # 방법3: data-cid (place id)
            cids=re.findall(r'data-cid="(\d+)"',html)
            # 방법4: 큰 JSON 블록에서 place list 찾기
            # "commonAddress" 또는 "roadAddress" 근처에 업체 데이터
            addr_blocks=re.findall(r'"(?:commonAddress|roadAddress)"\s*:\s*"([^"]+)"',html)
            # 방법5: tel + name 근처 블록
            tel_blocks=re.findall(r'"virtualTel"\s*:\s*"([^"]+)"',html)
            vtel2=re.findall(r'"tel"\s*:\s*"([^"]*)"',html)
            # 방법6: place/숫자 링크 + 업체명
            place_links=re.findall(r'place/(\d{5,})[^"]*"[^>]*>([^<]{2,30})<',html)
            # 방법7: 전체 JSON 객체 중 id+name+tel 조합
            full_items=re.findall(r'\{"id"\s*:\s*"(\d{5,})"[^}]*"name"\s*:\s*"([^"]+)"[^}]*?"(?:tel|virtualTel)"\s*:\s*"([^"]*)"[^}]*?"(?:address|roadAddress|commonAddress)"\s*:\s*"([^"]*)"',html)
            # 방법8: reviewCount 근처 블록
            review_blocks=re.findall(r'"reviewCount"\s*:\s*(\d+)',html)
            # 방법9: category 근처
            cat_blocks=re.findall(r'"category"\s*:\s*"([^"]+)"',html)
            return {"html_len":len(html),"id_name_pairs":id_name[:30],"data_cids":list(set(cids))[:20],"addresses":addr_blocks[:10],"virtual_tels":tel_blocks[:10],"tels":vtel2[:10],"place_links":place_links[:20],"full_items":full_items[:20],"review_counts":review_blocks[:20],"categories":cat_blocks[:20],"typename_blocks":[b[:200] for b in places_json[:5]]}
    except Exception as e:
        return {"error":str(e)}
...
async def debug_naver(keyword:str):
    ...
    tests=[
        ("m.search.naver.com",f"https://m.search.naver.com/search.naver?query={urllib.parse.quote(keyword)}&sm=mtb_plc&where=m_local",{"User-Agent":mobile_ua,"Accept":"text/html"}),
        ("map.naver.com/p/api",f"https://map.naver.com/p/api/search/allSearch?query={urllib.parse.quote(keyword)}&type=place&searchCoord=126.9783882%3B37.5666103&boundary=",NAVER_HEADERS),
        ("pcmap-api graphql POST",gql_url_const,None),
    ]
    for name,url,hdrs in tests:
        try:
            async with httpx.AsyncClient(proxy=p,timeout=15,follow_redirects=True) as c:
                if name.startswith("pcmap"):
                    payload=[{"operationName":"getPlacesList","variables":{"input":{"query":keyword,"start":1,"display":3,"adult":False,"spq":False,"queryRank":"","x":"126.9783882","y":"37.5666103","deviceType":"pcmap","bounds":""},"isNmap":True,"isBounds":False},"query":"query getPlacesList($input: PlacesInput, $isNmap: Boolean!, $isBounds: Boolean!) { businesses: places(input: $input) { total items { id name tel category address reviewCount } } }"}]
                    r=await c.post(url,json=payload,headers={"Content-Type":"application/json","User-Agent":UA,"Referer":"https://pcmap.place.naver.com/","Origin":"https://pcmap.place.naver.com"})
                else:
                    r=await c.get(url,headers=hdrs)
                results.append({"name":name,"status":r.status_code,"len":len(r.text),"body":r.text[:800]})
        except Exception as e:
            results.append({"name":name,"error":str(e)})
    # direct (no proxy) m.search
    try:
        async with httpx.AsyncClient(timeout=15,follow_redirects=True) as c:
            r=await c.get(f"https://m.search.naver.com/search.naver?query={urllib.parse.quote(keyword)}&sm=mtb_plc&where=m_local",headers={"User-Agent":mobile_ua})
            results.append({"name":"direct-m.search","status":r.status_code,"len":len(r.text),"body":r.text[:800]})
    except Exception as e:
        results.append({"name":"direct-m.search","error":str(e)})
