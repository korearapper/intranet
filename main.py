...
async def naver_search(keyword: str, proxy: str):
    ...
    urls = [
        f"https://map.naver.com/p/api/search/allSearch?query={urllib.parse.quote(keyword)}&type=place&searchCoord=&boundary=",
        f"https://map.naver.com/v5/api/search?caller=pcweb&query={urllib.parse.quote(keyword)}&type=place&searchCoord=&boundary=",
    ]
    for url in urls:
        try:
...
async def debug_naver(keyword:str):
    p=px()
    urls=[
        f"https://map.naver.com/p/api/search/allSearch?query={urllib.parse.quote(keyword)}&type=place",
        f"https://map.naver.com/v5/api/search?caller=pcweb&query={urllib.parse.quote(keyword)}&type=place",
    ]
    results=[]
    for url in urls:
        try:
